package eas.miscellaneous.gnuplot;
...
public class JGnuplot extends Logger implements Serializable {
    private static final long serialVersionUID = -5274961608716373028L;
	...
	private static FileLocationEstimator estimator = new FileLocationEstimator(
	        ".*program.*", ".*gnuplot.*", "bin", ".*gnuplot.exe.*");
	
	protected String sCommand(String sFile, boolean bPause) {
	    String gnuPlotLocation = ExternalFilePathsManager.retrieveExternalFilePath(
	            ExternalFilePathsManager.PATH_TO_GNUPLOT_ID, 
	            true,
	            "Choose the file path of the gnuplot executable (usually gnuplot.exe).",
	            estimator,
	            null,
	            null,
	            false).getAbsolutePath();
	    
		String rtn = gnuPlotLocation + " " + sFile;
		if (bPause) {
